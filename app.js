...
async function exportOpnameCsv(){
  ...
  if (!ok) return;
  const arr = await getAllOpname();
  if(arr.length === 0){ showAlert('error', 'Tidak ada data untuk diexport'); return; }
  ...
  a.href = url;
  a.download = `opname_export_${new Date().toISOString().slice(0,10)}.csv`;
  a.click();
...
async function openScannerModal(){
  // Try ZXing first (if library loaded)
  const hasZXing = (typeof ZXing !== 'undefined') || (typeof BrowserMultiFormatReader !== 'undefined');
  if(hasZXing){
    try{
      // ensure modal visible
      $('modalScanner').classList.remove('hidden');
      const videoElem = $('scannerVideo');

      // Create reader with whichever global is available
      try {
      ...
      if(!codeReader){
        // fallback to BarcodeDetector handling below
        throw new Error('ZXing reader not available');
      }

      // list devices and choose environment camera if possible
      const devices = await codeReader.listVideoInputDevices();
      let chosenDeviceId = null;
      if(devices && devices.length){
        // prefer device with "back" or "rear" in label
        chosenDeviceId = devices.find(d => /back|rear|environment/i.test(d.label))?.deviceId || devices[0].deviceId;
      }

      // start decoding from selected device into video element
      // decodeFromVideoDevice will attach stream to video element internally
      scannerStream = null; // we let ZXing manage stream
      scannerRunning = true;

      codeReader.decodeFromVideoDevice(chosenDeviceId || null, videoElem, (result, err) => {
        if(result){
          // got code
          const code = result.text || result.rawValue || '';
          // stop reader
        ...
        // ignore err per-frame
      });

      return;
    }catch(err){
      // if ZXing failed, fallthrough to BarcodeDetector approach
      console.warn('ZXing scanner failed, fallback to BarcodeDetector or manual input', err);
      // ensure modal visible for fallback
      $('modalScanner').classList.remove('hidden');
    }
  }

  // Fallback: use native BarcodeDetector (if available)
  if('BarcodeDetector' in window){
    ...
    try {
      ...
      scannerRunning = true;
      const detector = new BarcodeDetector({ formats: ['ean_13','ean_8','code_128','qr_code','upc_e','upc_a'] });
      const scanLoop = async () => {
        ...
        try{
          ...
          // detection might throw if video not ready; ignore
        }
  ...
  // Last fallback: ask user to input manually
  const ok = confirm('Perangkat atau browser Anda tidak mendukung scanner otomatis. Gunakan input manual? (OK untuk manual)');
  if(ok){
    $('modalScanner').classList.add('hidden');
    $('inputUPC').focus();
  } else {
...
async function stopScannerAndClose(){
  ...
  // stop any media stream we created
  if(scannerStream){
    for(const t of scannerStream.getTracks()){
      try{ t.stop(); } catch(e){}
    }
    scannerStream = null;
  }
  // also try stop tracks attached to video element
  const video = $('scannerVideo');
